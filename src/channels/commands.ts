...
export const TELEGRAM_MENU_COMMANDS: BotCommandDef[] = [
  ...
  { command: "thoughts", description: "Reasoning on|off|minimal", menu: true },
  { command: "steps", description: "Tool activity on|off|minimal", menu: true },
  { command: "verbose", description: "Thoughts+steps on|off|minimal", menu: true },
...
export function helpText(agentName: string): string {
  const lines = [
    ...
    `/thoughts minimal  — italic Thinking… while reasoning`,
    `/steps on          — tool calls + results as they happen`,
    `/steps minimal     — tool name only (no params/results)`,
    `/verbose on        — full thoughts + full steps`,
    `/verbose minimal   — Thinking… + tool names only`,
    `/skills`,
